...
async function main() {
  ...
  const livePricingMapByKey = new Map(); // key -> array of records
  ...
  for (const seedPrice of seed.pricing) {
    ...
    // 1. Try match by ID
    let livePrice = livePricingMapById.get(seedPrice.id);
    
    // 2. Fallback to key match if ID match not found or service/variant is different
    if (!livePrice || livePrice.service_id !== liveSvcId || livePrice.variant_id !== liveVariantId) {
      ...
      livePrice = candidates.find(c => !usedLiveIds.has(c.id));
    }
  ...
  console.log(`\n=== Re-Analysis Results (Exact ID mapping + Key fallback) ===`);
