...
export async function createClient() {
  try {
    ...
    // Fallback for background scripts/CLI where cookies() is not available
...
export async function createClient() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    if (!hasWarned) {
      console.warn('Supabase server environment variables are missing. Using mock client for build/pre-rendering.');
      hasWarned = true;
    }
    return dummyClient;
  }
