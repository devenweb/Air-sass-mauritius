...
async function main() {
  ...
  for (const seedPrice of seed.pricing) {
    ...
    // We only check if variant_id was mapped or not, let's keep it simple for checking
    let liveVariantId = seedPrice.variant_id;
    // (Assume roomTypeMapping is not needed for showing a few diffs, or we can just lookup key)
