...
async function createPricingGridEntries() {
  console.log("Creating updated pricing grid entries for services with service role key...");
    ...
    for (const pricing of pricingRecords) {
      ...
      for (const { variation, labelSuffix } of variations) {
        ...
        const newPricing = {
          ...
          // Include the newly added columns if they exist in the original record
  ...
  console.log("Completed creating updated pricing grid entries for services");
}

// Run the function
createPricingGridEntries()
  .then(() => console.log('Updated pricing grid creation process completed'))
  .catch(error => console.error('Error during updated pricing grid creation:', error));