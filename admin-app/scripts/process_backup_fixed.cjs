 ...
 */
async function processBackupFilesFixed() {
  console.log("Processing backup files (fixed approach) to create new services with different pricing...\n");
  ...
  for (const backupDir of backupDirs) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(2, serviceMatches.length); i++) {
        ...
        // The values correspond to: id, name, description, etc.
        // We need to skip the ID field when creating a new service
        if (values.length >= 3) {
          ...
          try {
            ...
            // Now find pricing records for the original service in the SQL and create variations
            // Look for service_pricing entries that reference the original service ID
            ...
            for (let j = 0; j < Math.min(3, pricingMatches.length); j++) { // Limit to 3 pricing records
              ...
              // Expected structure: id, service_id, variant_id, label, date_from, date_to, price, currency, price_type, ...
              if (pValues.length >= 7) {
