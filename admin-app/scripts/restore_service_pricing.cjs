...
const fsextra = require('fs'); // Need this for existsSync
...
async function parseSeedFile(filePath) {
  ...
  // Updated regex to match INSERT statements with potential ON CONFLICT clauses and public. prefix
  const insertRegex = /INSERT INTO (?:public\.)?"?(?<table>\w+)"?\s*(?:\((?<columns>[^)]+)\))?\s+VALUES\s+(?<values>[\s\S]+?)(?=;(?:\s*--|$)|(?:ON\s+CONFLICT)|(?=INSERT\s+INTO)|$)/gi;
  ...
  while ((match = insertRegex.exec(content)) !== null) {
    ...
    let valuesStr = match.groups.values;
    
    // Handle ON CONFLICT clauses - remove them from the values string
    valuesStr = valuesStr.split(/\s+ON\s+CONFLICT/i)[0];
    ...
    // Parse the values - handle multiple value groups in one INSERT statement
...
async function restoreServicePricing() {
  ...
  // Construct path to the backup directory using absolute path
  const currentDir = __dirname;
  const adminAppDir = path.dirname(currentDir); // Go up to admin-app
  const appsDir = path.dirname(adminAppDir); // Go up to apps
  const webAppDir = path.join(appsDir, 'web-app');
  const backupDir = path.join(webAppDir, 'supabase', 'backups', 'sql_2026-05-15');
  const seedFile = path.join(backupDir, 'seed_2026-05-15.sql');
  
  try {
    console.log(`📖 Reading backup data from: ${seedFile}`);
    console.log(`📁 Backup directory exists: ${fsextra.existsSync(backupDir)}`);
    console.log(`📄 Seed file exists: ${fsextra.existsSync(seedFile)}`);
    
    if (!fsextra.existsSync(seedFile)) {
